...
```kotlin
fun main() {
    println("My name is huangtao")
    println("My student ID is 2025003008")
...
```kotlin
fun main() {
    println("Student 2025003008 says hello")
} //错误原因：缺少右大括号,main函数没有闭合
```
...
```kotlin
fun main() {
    println("Hello, my name is huangtao")
}//错误原因：输出函数使用错误，将printline 修改为 println
```
...
```kotlin
fun main() {
    println("ID: 2025003008");println("Name: huangtao");println("Course: Kotlin")
}//错误原因：多个换行语句需要写在同一行 需要使用分号';' 去分隔
```
...
fun main() {
    ...
    println("You have ${"%02d".format(numberOfMessages)} unread messages.")
}//kotlin中不能在整数数字前面使用0 会导致语法解析错误
//我们将数字进行格式化后 进行输出
```
...
```kotlin
fun main(){
    val operandOne = 0 //kotlin中整数前前导不能为0 所以在输出的时候需要使用format 进行格式化进行输出
    val operandTwo = 8
    println("${"%02d".format(operandOne)} + $operandTwo = ${operandOne + operandTwo}")
    println("${"%02d".format(operandOne)} - $operandTwo = ${operandOne - operandTwo}")
    println("${"%02d".format(operandOne)} / $operandTwo = ${operandOne / operandTwo}")
    println("${"%02d".format(operandOne)} * $operandTwo = ${operandOne * operandTwo}")
    println("${"%02d".format(operandOne)} % $operandTwo = ${operandOne % operandTwo}")
