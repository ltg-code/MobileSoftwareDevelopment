...
data class Event(
    ...
    val description: String? = null, // 活动说明
    val timeOfDay: String,      // 活动时段（上午/下午/晚上）
    val durationMinutes: Int    // 活动时长（分钟，整数）
)
...
fun main() {
    ...
    val meeting = Event(
        title = "项目周会",
        description = "讨论本周进度与问题",
        timeOfDay = "下午",
        durationMinutes = 60
    )
    // 创建一个不带描述的活动（description 自动为 null）
    val workout = Event(
        title = "健身房训练",
        timeOfDay = "晚上",
        durationMinutes = 100
...
data class Event(
    val title: String,          // 活动标题
    val description: String? = null, // 活动说明
    val daypart: Daypart,      // 活动时段（上午/下午/晚上）
    val durationMinutes: Int    // 活动时长（分钟，整数）
)
...
data class Event(
    ...
    val description: String? = null, // 活动说明
    val daypart: Daypart,      // 活动时段（上午/下午/晚上）
    val durationInMinutes: Int    // 活动时长（分钟，整数）
)
...
fun main() {
    // 查看活动总数
    println("总活动数：${events.size}")

    // 添加一个新活动
    events.add(
        Event(title = "Read a book", daypart = Daypart.EVENING, durationInMinutes = 30)
    )
    println("添加后活动数：${events.size}")
}
```
